...
DeviceClient.prototype.unsubscribeAll = function(serviceId) {
  var self = this;
  serviceId = resolveService(serviceId);
  ...
  // Remove the listener from the list
  subscription.listeners.splice(0, subscription.listeners.length);
...
function parseEvents(buf) {
  ...
  var systemUpdateId = doc.findtext('.//SystemUpdateID');
  if(systemUpdateId) {
    var data = {};
    data["SystemUpdateID"] = systemUpdateId;
    events.push(data);
  };
  
  //console.log("xxxxxxxxxxxxxxxxxxxxxx");
  var containerUpdateIds = doc.findtext('.//ContainerUpdateIDs');
  if(containerUpdateIds) {    
    var data = {};
    data["ContainerUpdateIDs"] = containerUpdateIds;
    events.push(data);
  };
