...
module.exports = class MediaListManager extends ManagerBase
{
    ...
    */
    ...
    loadMediaListForRendererUri(_rendererUdn, _uri, _uriMetadata, _useCache = false, _dataPackageCount = 25, _dataPackageCallback = null)
    {
        ...
        // sometimes a "false" is returned to say hey, i have no metadata!
        if(_uriMetadata == "false")
            _uriMetadata = "";
    ...
    getMediaList(_listId, _objectId, _searchCriteria = "", _useListCache = true, _emitReady = true, _dataPackageCount = 25, _dataPackageCallback = null)
    {
        var self = this;        
        this.logVerbose("Get media list for objectId: " + _objectId);                    
        ...
        return new Promise(function(_resolve, _reject){
            try
            {
                ...
                self.listCache.delete(_listId);
                self.getMediaList_Rec(_listId, _objectId, _searchCriteria, _emitReady, _dataPackageCount, _dataPackageCallback, 0).then(function(_data){
                    if(_emitReady)
                        self.emit("mediaListDataReady", _listId, _data);
                    _resolve(self.listCache.get(_listId));
                }).catch(function(_data){
    ...
    getMediaList_Rec(_listId, _objectId, _searchCriteria, _emitReady, _dataPackageCount, _dataPackageCallback, _pkgIdx)
    {
        var self = this;        
        ...
        return new Promise(function(_resolve, _reject){
            try
            {    
                // we do use "search" instead of browse s owe cann use this method for searching too
                // the result is the same so it does not matter. If there are problems we will have to jump in with an if (browse and search)
                //self.mediaServer.search(_objectId, _searchCriteria, "*", _pkgIdx, _dataPackageCount).then(function(_data){
                self.mediaServer.browse(_objectId, "BrowseDirectChildren", "*", _pkgIdx, _dataPackageCount).then(function(_data){
                    ...
                    mediaDataConverter.convertXMLToMediaList(_data).then(function(_data){
                        ...
                        {
                            self.getMediaList_Rec(_listId, _objectId, _searchCriteria, _emitReady, _dataPackageCount, _dataPackageCallback, (_pkgIdx + curPackageDataCount)).then(function(_data){
                                _resolve(_data);
                            }).catch(function(_data){
                                _reject(_data);
                            });
    ...
    /**
     * may be used to search on a special "search" container id
     * Examples of special search containers are:
     *   0/My Music/Search/Albums
     *   0/Napster/Search/Artist
     *   0/RadioTime/Search
     * @param {string} a list id where the result will be stored in
     * @param {string} a special search container id 
     * @param {string} the search value
     */
    searchMediaList(_listId, _searchContainerId, _searchValue = "", _useListCache = true, _emitReady = true, _dataPackageCount = 25, _dataPackageCallback = null)
    {
        /*  search only works on "search" containers e.g.:
            0/My Music/Search/Albums
            0/Napster/Search/Artist
            0/RadioTime/Search
            ...
            So in fact a client has to check the 2nd level for a "Search" node
        */
        var searchCriteria = _searchValue ? 'dc:title contains "' + _searchValue + '"' : '';        
        return this.getMediaList(_listId, _searchContainerId, searchCriteria, _useListCache, _emitReady, _dataPackageCount, _dataPackageCallback);
    }  
